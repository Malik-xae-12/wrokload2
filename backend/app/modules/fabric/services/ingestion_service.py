...
from app.modules.fabric.models.ingestion_models import TableSyncJob, SyncJobRun, MigrationProject
...
async def save_or_update_jobs(
    ...
    effective_pid = project_id or (jobs_payload[0].project_id if jobs_payload else None)

    # Clean up jobs for this project that were unselected by the user
    if effective_pid:
        current_keys = {(item.source_schema, item.source_table) for item in jobs_payload}
        all_pjobs_stmt = select(TableSyncJob).where(TableSyncJob.project_id == effective_pid)
        all_pjobs_res = await db.execute(all_pjobs_stmt)
        for pj in all_pjobs_res.scalars().all():
            if (pj.source_schema, pj.source_table) not in current_keys:
                await db.delete(pj)
        await db.commit()
...
async def create_project(db: AsyncSession, payload: MigrationProjectCreate) -> MigrationProject:
    """Create a new migration project record."""
...
async def list_projects_with_stats(db: AsyncSession) -> list[MigrationProjectRead]:
    """List all projects along with aggregated live metrics and sync status."""
    stmt = select(MigrationProject).order_by(MigrationProject.created_at.desc())
    res = await db.execute(stmt)
    projects = res.scalars().all()

    output: list[MigrationProjectRead] = []
    for proj in projects:
        # Load jobs for this project
        job_stmt = select(TableSyncJob).where(TableSyncJob.project_id == proj.id)
        ...
        output.append(
            MigrationProjectRead(
                ...
                source_server=proj.source_server,
                source_database=proj.source_database,
                source_username=proj.source_username,
                source_port=proj.source_port,
                target_workspace_id=proj.target_workspace_id,
                target_workspace_name=proj.target_workspace_name,
                target_lakehouse_name=proj.target_lakehouse_name,
                target_warehouse_name=proj.target_warehouse_name,
                target_server=proj.target_server,
                target_database=proj.target_database,
                ...
                jobs=[TableSyncJobRead.from_orm(j) for j in jobs],
            )
        )
    return output
...
async def get_project_by_id(db: AsyncSession, project_id: str) -> MigrationProjectRead | None:
    """Retrieve a single project by ID with its jobs."""
...
async def update_project_by_id(
    db: AsyncSession, project_id: str, payload: MigrationProjectUpdate
...
async def delete_project_by_id(db: AsyncSession, project_id: str) -> bool:
    """Delete project and cascade delete all associated sync jobs."""
    stmt = select(MigrationProject).where(MigrationProject.id == project_id)
    res = await db.execute(stmt)
    proj = res.scalar_one_or_none()
    if not proj:
